            ...
            if i+1 < len(j):
                for r2 in resource_m['R'+str(i+1)]:
                    ...
                    for k1 in range(lb, ub):
                        ...
                        for k2 in range(fd, ub):  # 修正: fd から ub までの範囲に変更
                            #if k2 < time:  # 修正: time の範囲内かどうか確認
                ...
                for k2 in range(lb2,ub2):
                    for r2 in resource_m['R'+str(i)]:
                        ...
                        #     H_waitingTime += r_num1*x[k1,t1,r_num1] - fd*x[k2,t2,r_num2]
                            H_waitingTime += k1*x[k1,t1,r_num1] - fd*x[k2,t2,r_num2]
...
#wating timeでパフォーマンス resource costでコスト

# モデルをコンパイル
model = H.compile()

feed_dict = {"A": 420.0, "B": 35.0, "C": 10.0, "D": 1, "E": 0.5}
...
def calculate_subh_energy(subh, sample, feed_dict):
    ...
    print("subh_offset :",subh_offset)
    print("subh_energy :",subh_energy)
    return subh_energy + subh_offset

# # # 最良のサンプルの各サブハミルトニアンの値を表示
# # print("Best Sample:", best_sample.sample)
# print("SubH1 (A * H_firing):", best_sample.subh['SubH1'])
# print("SubH2 (B * H_conflict):", best_sample.subh['SubH2'])
# print("SubH3 (C * H_precedence):", best_sample.subh['SubH3'])

# # 最良のサンプルの各サブハミルトニアンの値を表示
print("Best Sample:", best_sample.sample)
print("SubH1 (A * H_firing):", calculate_subh_energy(A * H_firing, best_sample.sample, feed_dict))
print("SubH2 (B * H_conflict):", calculate_subh_energy(B * H_conflict, best_sample.sample, feed_dict))
print("SubH3 (C * H_precedence):", calculate_subh_energy(C * H_precedence, best_sample.sample, feed_dict))
print("D * H_resourceCost:", calculate_subh_energy(D * H_resourceCost, best_sample.sample, feed_dict))
# print("E * H_waitingTime:", calculate_subh_energy(E * H_waitingTime, best_sample.sample, feed_dict))

resourceCost = calculate_subh_energy(D * H_resourceCost, best_sample.sample, feed_dict)
watingTime = calculate_subh_energy(E * H_waitingTime, best_sample.sample, feed_dict)
